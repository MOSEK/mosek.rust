...
impl Task
{
    ...
    pub fn generate_djc_names(& mut self,sub_ : & [i64],fmt_ : &str,dims_ : & [i32]) -> Result<(),String>
    {
      let mut num_ = sub_.len();
      let mut ndims_ = dims_.len();
      let call_res = unsafe { (MSK_generatedjcnames(self.ptr,num_ as i64,sub_.as_ptr(),CString::new(fmt_).unwrap().as_ptr(),ndims_ as i32,dims_.as_ptr())) };
